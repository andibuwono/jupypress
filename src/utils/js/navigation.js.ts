/**
 * Navigation JavaScript — HTML Presentation Framework.
 * Source: reference/static/js/navigation.js
 */
...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    /* URL Hash — silently skipped inside srcdoc/sandboxed iframes */
    function updateHash() {
      try {
    ...
    /* ── postMessage bridge ─────────────────────────────────────────── */
    /* Parent → iframe: navigate to a specific slide index              */
    /* Parent → iframe: set kernel connection status                    */
    /* iframe → parent: execute code cell source, receive output        */
    ...
    function escapeAttr(str) {
      return escHtml(str).replace(/\x60/g, '&#96;');
    }
